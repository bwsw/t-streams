...
class CommitLogQueueBootstrapTestSuite
  ...
  private def getOrderedFiles(orderedQueue: PriorityBlockingQueue[CommitLogStorage]) = {
    @tailrec
    def inner(orderedFiles: Seq[CommitLogStorage]): Seq[CommitLogStorage] = {
      Option(orderedQueue.poll()) match {
        case Some(path) => inner(orderedFiles :+ path)
        case None => orderedFiles
      }
    }
