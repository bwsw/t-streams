...
trait Installer {
  ...
  def startTransactionServer(): Boolean = {
  ...
  def createStream(name: String, partitions: Int): Int = {
    ...
    val streamID =
      if (Await.result(client.checkStreamExists(name), 5.seconds)) {
        Await.result(client.getStream(name), 5.seconds)
          .map(_.id)
          .getOrElse(throw new IllegalArgumentException("Something wrong with stream"))
      } else {
