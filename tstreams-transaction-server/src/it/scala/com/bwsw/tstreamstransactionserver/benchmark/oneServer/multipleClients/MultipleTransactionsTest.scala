...
import com.bwsw.tstreamstransactionserver.benchmark.utils.writer.TransactionMetadataWriter
  ...
  override def launchClients(streamID: Int): Unit = {
    (1 to clients).foreach(x => {
      val thread = new Thread(() => {
        val filename = s"${Random.nextInt(100)}_${txnCount}TransactionMetadataWriterOSMC.csv"
