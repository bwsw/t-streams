...
object SingleNodeServerOptions {
  ...
  /** The options are applied on creation Rocksdb database.
    ...
    *                                   Used for [[com.bwsw.tstreamstransactionserver.netty.server.ServerHandler]]
    * @param transactionTtlAppendSec    the value to sum with a stream [[com.bwsw.tstreamstransactionserver.rpc.StreamValue.ttl]] attribute, to determine lifetime of all producer transactions data belonging to the stream.
    * @param transactionExpungeDelaySec the lifetime of a producer transaction after persistence to database.(default: 6 months). If negative integer - transactions aren't deleted at all.
    ...
    * @param compactionInterval         seconds between RocksDB storage compaction
    */
