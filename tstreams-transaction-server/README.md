...
| rocksdb.transaction-ttl-append-sec                | The value to sum with a stream TTL attribute, to determine lifetime of all producer transactions data belonging to the stream. |int    | 50| [1,...]|
| rocksdb.transaction-expunge-delay-sec             | The lifetime of a producer transaction after persistence to database.(default: 6 months). If negative integer - transactions aren't deleted at all. |int    |259200 | integer |
...
| rocksdb.compaction-interval                       | Seconds between RocksDB storage compaction. It's need to delete expired entries from RocksDB (e.g. expired transaction). |integer|3600|positive integer|
