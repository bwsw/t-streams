...
class LocalNodeFirstLoadBalancingPolicy(contactPoints: Set[InetSocketAddress], localDC: Option[String] = None,
                                        shuffleReplicas: Boolean = true) extends LoadBalancingPolicy {
...
object LocalNodeFirstLoadBalancingPolicy {
  ...
  def sortNodesByStatusAndProximity(contactPoints: Set[InetSocketAddress], hostsToSort: Set[Host]): Seq[Host] = {
